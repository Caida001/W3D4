...
def harrison_ford
  ...
  Movie.joins(:castings).joins(:actors).where("actors.name = 'Harrison Ford' AND castings.actor_id = actors.id AND castings.ord > 1")
  .select(:id, :title)
end
...
def directed_by_one_of(them)
  ...
  Movie.where(director: Actor.where(name: them) )
    .select('movies.id, movies.title')
